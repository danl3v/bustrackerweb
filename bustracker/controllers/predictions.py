...
class Predictions(webapp.RequestHandler):
    def get(self):
        ...
        self.response.out.write(json.dumps([{"id": stop.key().id(), "title": stop.title, "timeToStop": stop.time_to_stop, "directions": get_directions(stop), "position": stop.position} for stop in stops]))
